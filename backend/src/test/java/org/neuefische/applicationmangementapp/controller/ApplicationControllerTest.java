...
class ApplicationControllerTest {
    ...
    void getAllApplications() throws Exception {
        applicationRepo.saveAll(List.of(new Application("test", "tester", "desc", "resume", appliStatus.OPEN, null, LocalDate.now()), new Application("test2", "tester2", "desc", "resume", appliStatus.OPEN, null, LocalDate.now())
    ...
    void getApplicationById() throws Exception {
        applicationRepo.saveAll(List.of(new Application("test", "tester", "resume", "cl", appliStatus.OPEN, null, LocalDate.of(2025, 2, 20)), new Application("test2", "tester2", "desc", "resume", appliStatus.OPEN, null, LocalDate.of(2025, 5, 6))));
    ...
    void updateApplication() throws Exception {
        ...
        mockMvc.perform(put("/api/application/" + "test1").contentType(MediaType.APPLICATION_JSON).content("""
                {
                 "jobOfferID": "tester2",
                 "resume": "resume",
    ...
    void tryUpdateApplicationByIdShoudFail() throws Exception {
        String updatedApplication = """
                ...
                """;
        mockMvc.perform(put("/api/joboffer/" + "test").contentType(MediaType.APPLICATION_JSON).content(updatedApplication)).andExpect(status().isNotFound()).andExpect(content().json("""
    ...
    void expectSuccessfulPost() throws Exception {
        String actual = mockMvc.perform(post("/api/application").contentType(MediaType.APPLICATION_JSON).content("""
